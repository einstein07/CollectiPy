#!/bin/bash
# =============================================================================
# mean-field-flexibility sweep — asymmetric target quality × neural gain characterization
# bwUniCluster3.0 SLURM job array
#
# Purpose: flexibility analysis of ring attractor with SFA decision-making across three
# values of the neural gain parameter u (5, 6.156868, 8).  For each u, the
# arena is asymmetric: static_0 is held fixed at strength 1.0 while static_1
# is weakened by a quality difference δ ∈ [0%, 80%]. The targets are then swapped after
# the bifurcation event. The 22 δ values are:
...
#   bash submit-mean-flexibility-sweep-sfa.sh
#
...
#SBATCH --job-name=mean_field_flexibility_sweep_sfa
...
CONFIG_TEMPLATE="$PROJECT_DIR/config/mean_field_2_targets_no_viz_sfa.json"
LOGS_DIR="/pfs/work9/workspace/scratch/kn_pop547841-mySpace/collectipy-data/mean_field_flexibility_sweep_sfa"
# ---------------------------------------------------------------------------
# Sweep parameters
# ---------------------------------------------------------------------------
# Three neural gain values for flexibility analysis
U_VALUES=(7.5 7.8 10)
...
if [ -z "${SLURM_ARRAY_TASK_ID:-}" ]; then
    echo "mean-field-flexibility sweep — asymmetric arena, varying quality difference, swap after bifurcation"
    ...
    sbatch \
        ...
        --output="${LOGS_DIR}/u_flexibility_%A_%a.out" \
        --error="${LOGS_DIR}/u_flexibility_%A_%a.err" \
...
for RUN_ID in $(seq "$FIRST_RUN" "$LAST_RUN"); do
    CONFIG_OUT="$TMP_DIR/u_flexibility_u${U_VALUE}_diff${DIFF}_run${RUN_ID}.json"
...
# static_0 fixed at 1.0; static_1 weakened by quality difference δ.
...
# Set u for all agents that have a mean_field_model block.
...
PYEOF
    ...
    "$PYTHON_BIN" "$PROJECT_DIR/src/main.py" -c "$CONFIG_OUT"
done

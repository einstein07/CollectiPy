...
"environment":{
    ...
    "target_position_swaps": list(dict), DEFAULT:[] optional run-time target position swaps (executed by arena tick):
        each entry supports "at_tick": int >= 0 or "at_seconds": float >= 0
        "pairs": list([target_id_a, target_id_b]) or list({"a": "...", "b": "..."})
        target IDs must match runtime object names (for example "static_0.s#0")
    "results":{ DEFAULT:{} empty dict -> no saving. If rendering is enabled -> no saving
        ...
        "agent_specs": list(str) DEFAULT:[] - enable per-agent exports:
            ...
            "mean_field" -> export neural/perception/sensory/target/position logs.
